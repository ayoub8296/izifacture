...
import { createQuote, generateNextQuoteNumber } from '@/lib/services/quotes';
...
export default function NewQuotePage() {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    {
      id: `item-${Date.now()}-1`,
      description: 'Développement d’application web sur mesure & intégration API',
      quantity: 1,
      unitPrice: 1200000,
      totalPrice: 1200000,
    },
  ...
  useEffect(() => {
    async function init() {
      ...
      const nextNum = await generateNextQuoteNumber();
  ...
  const handleSubmit = async (status: 'draft' | 'sent') => {
    ...
    try {
      ...
      router.push(`/facturation/devis/${newQ.id}`);
    } catch (err) {
      console.error(err);
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/facturation/devis" className="p-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 text-slate-500 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm touch-manipulation">
          ...
          <Card className="border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
            <CardHeader className="pb-3 border-none">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                <span>Destinataire (Prospect / Client)</span>
              </CardTitle>
          ...
          <Card className="border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
            <CardHeader className="pb-3 border-none">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                <span>Dates de validité</span>
              </CardTitle>
              <CardDescription>Période de validité de l’offre</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 pt-0">
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700 rounded-xl text-xs font-medium text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-pink/30 focus:border-brand-pink"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1.5">
                  Valable jusqu’au <span className="text-rose-500">*</span>
                </label>
                <input
                  type="date"
                  value={validityDate}
      ...
      <motion.div variants={itemVariants}>
        <Card className="border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
            <div>
              <CardTitle className="text-base">Prestations & Offres détaillées</CardTitle>
              <CardDescription>Détaillez les modules, services ou produits</CardDescription>
            </div>
            <Button
              type="button"
              variant="outline"
          ...
          <CardContent className="p-0">
                      ...
                      >
                        <td className="py-3 px-6">
                          <input
                            ...
                            className="w-full px-3 py-1.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-medium text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-brand-pink focus:border-brand-pink focus:bg-white dark:focus:bg-slate-800"
                          />
                        </td>
            ...
            <div className="p-6 bg-slate-50/70 dark:bg-slate-950/60 border-t border-slate-200/70 dark:border-slate-800 flex flex-col md:flex-row justify-between items-start gap-6">
              <div className="text-xs text-slate-500 dark:text-slate-400 max-w-md space-y-1">
                ...
                <p>Offre valable 30 jours à compter de la date d’émission. Acompte de 50% au lancement du projet.</p>
              </div>
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-end gap-3 pt-2">
        ...
        >
          <Send className="w-4 h-4" />
          <span>Créer & Envoyer le devis</span>
        </Button>
      </motion.div>
    </motion.div>
  );
